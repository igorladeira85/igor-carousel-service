...
def generate_slide(title, body, slide_num, total_slides,
    ...
    if body:
        ...
        draw_text_wrapped(draw, body, INNER, y, TEXT_W, f_body, t["BODY_C"], line_spacing=16)
    ...
    cb = draw.textbbox((0, 0), counter, font=f_footer)
    draw.text((W - PAD - (cb[2] - cb[0]), footer_y + 12), counter, fill=t["FOOTER_C"], font=f_footer)
    return img
...
def generate_slide_lista(title, items, slide_num, total_slides,
    ...
    for item in items:
        ...
        r = 22
        cx2, cy2 = INNER + r, y + r
        draw.ellipse([(cx2-r, cy2-r), (cx2+r, cy2+r)], fill=t["ACCENT"])
        draw.text((cx2-(nb[2]-nb[0])//2, cy2-(nb[3]-nb[1])//2-2), n_text, fill=t["TAG_FG"], font=f_num)
        ix = INNER + r*2 + 16
        iw = TEXT_W - r*2 - 16
        y_before = y
...
def generate_slide_grid(title, items, slide_num, total_slides,
    ...
    for i, item in enumerate(items):
        col = i % 2
        row = i // 2
        cx3 = INNER + col * (CELL_W + GAP)
        cy3 = y + row * (CELL_H + GAP)
        draw_rounded_rect(draw, (cx3, cy3, cx3+CELL_W, cy3+CELL_H), 12, t["CARD_BG"])
        draw.rectangle([(cx3, cy3, cx3+6, cy3+CELL_H)], fill=t["ACCENT"])
        n_text = item.get("num", f"{i+1:02d}")
        nb = draw.textbbox((0, 0), n_text, font=f_num_big)
        draw.text((cx3+14, cy3+8), n_text, fill=t["ACCENT_LIGHT"], font=f_num_big)
        ty = cy3 + 14 + (nb[3]-nb[1]) + 4
        ty = draw_text_wrapped(draw, item.get("title",""), cx3+14, ty, CELL_W-28, f_ctitle, t["BODY_C"], 2)
        if item.get("body"):
            draw_text_wrapped(draw, item["body"], cx3+14, ty+2, CELL_W-28, f_cbody, t["MUTED"], 2)
    footer_y = H - PAD - 50
    draw.rectangle([(INNER, footer_y), (W - PAD, footer_y + 1)], fill=t["ACCENT"])
    f_footer = fnt("inter", 26, 400)
    draw.text((INNER, footer_y + 12), handle, fill=t["FOOTER_C"], font=f_footer)
